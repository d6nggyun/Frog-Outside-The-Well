...
public interface DailyLogApiSpecification {
        ...
        ResponseEntity<Map<WeatherType, Long>> getThisWeekWeatherType(@AuthenticationPrincipal User user);

        @Operation(summary = "이번 달 WeatherType 통계", description = "사용자의 이번 달 DailyLogBefore 장소별 합계를 조회합니다.", responses = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "WeatherType 합계 조회")
        })
        ResponseEntity<Map<WeatherType, Long>> getThisMonthWeatherType(@AuthenticationPrincipal User user);
